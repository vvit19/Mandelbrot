...
Однако, во-первых, сразу появляется вопрос, насколько точны функции SFML (в частности, функция ```clock.getElapsedTime```) для вычисления времени.
...
Проверять точность SFML будем, сравнивая ее с функцией ```unsigned __int64 __rdtsc();``` из заголовочного файла ```intrin.h```. Эта функция (в дальнейшем будем называть ее идеальной) возвращает метку времени процессора. Метка времени процессора записывает количество циклов с момента последнего сброса. Сравнивать возвращаемые значения идеальной функции и функции ```clock.getElapsedTime``` будем при использовании AVX инструкций и флага ```-O3``` в исходной программе. Для достаточного числа данных для сравнения будем изменять число итераций цикла, в котором собственно и считается время. Пример цикла с использованием идеальной функции:
~~~ C++
...
И видим, что точки очень хорошо ложатся на аппроксимирующую прямую, при чем свободный член равен 2 миллисекундам, что является допустимой погрешностью для ```clock.getElapsedTime```. По коэффициенту наклона k можно найти тактовую частоту $&nu$ (вообще говоря, у современных процессоров она переменная, но, по всей видимости, во время исполнения нашей программы, ее колебания незначительны):
$&nu = 1/k = 1/0,4333 = 2,308$ ГГц.
